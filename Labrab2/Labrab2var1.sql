...
--+ 2. Информацию о продуктах, чье количество в заказе более 100
...
-- 5. Количество заказов по каждому клиенту(2 способа, через группировку и CROSS/OUTER APPLY), отсортировать по фамилии и имени клиента
...
--+ 6. Для каждого клиента определить общую стоимость по кажому продукту. Вывести фамилию имя клиента название продукци и его стоимость.
WITH sumSource AS (
...
-- 7. Поставщиков, которые поставляют только один товар (2 способа, с группировкой и без). Вывести имя компании, город, страну и наименование товаров.
;WITH IdProdName AS (
	SELECT SupplierId
		FROM dbo.Product
			GROUP BY SupplierId
...
-- 8. Количество каждого продукта в заказах и его общую стоимость. Вывести информацию о продукте и его количество.
;WITH sumSource AS(
